...
public class AppMain {
	...
	static public Veiculo buscarVeiculo(String nome, Seguradora seguradora){
		Cliente cliente = buscarCliente(nome, seguradora);
		if(cliente instanceof ClientePF){
			ClientePF cliente1 = (ClientePF) buscarCliente(nome, seguradora);
			System.out.print("\nQual a placa do carro que esta buscando:");
			String placa = lerStr();
			...
			ClientePJ cliente1 = (ClientePJ) buscarClientePJ(nome, seguradora);
			System.out.print("\nQual a placa do carro que esta buscando:");
			String placa = lerStr();
			for(Frota frota: cliente1.getListaFrota()){
			for(Veiculo veiculo: cliente1.getVeiculosPorFrota(frota)){
				if(veiculo.getPlaca().equals( placa))
				return veiculo;
			}
	...
	static public Cliente buscarCliente(String nome, Seguradora seguradora){
		for(Cliente clientes: seguradora.getListaClientes()){
			if (clientes.getNome().equals(nome) && clientes instanceof ClientePF)
				return  clientes;
	...
	static public ClientePF buscarClientePF(String nome, Seguradora seguradora){
		for(Cliente clientes: seguradora.getListaClientes()){
			if (clientes.getNome().equals(nome) && clientes instanceof ClientePF)
				return  (ClientePF) clientes;
			
			}
		return null;
	}

	static public ClientePJ buscarClientePJ(String nome, Seguradora seguradora){
		for(Cliente clientes: seguradora.getListaClientes()){
			if (clientes.getNome().equals(nome) && clientes instanceof ClientePJ)
				return (ClientePJ) clientes;
	...
	public static void lerClientePF(Seguradora seguradora){
		...
		seguradora.cadastrarCliente((Cliente) clientePf);
	}
	...
	public static void lerClientePJ(Seguradora seguradora){
		...
		seguradora.cadastrarCliente((Cliente) clientePj);
	}

	public static Condutor lerCondutor(Seguradora seguradora){
        //lê um clientepj e instancia
		System.out.print("\nCriar condutor");
		System.out.print("\nNome:");
		String nome = lerStr();
		...
		System.out.print("\nTelefone:");
		String telefone = lerStr();
		return new Condutor(cpf, LocalDate.parse(dataNasc), nome, endereco, telefone, email);
	}
	...
	public static void lerSeguroPf(){ 
		...
		Seguro seguro = new SeguroPF(id, LocalDate.parse(dataini), LocalDate.parse(datafim), Seguradora.buscarSeguradora(seguradoraString), lerVeiculo(Seguradora.buscarSeguradora(seguradoraString)),(ClientePF) buscarClientePF(cliente, Seguradora.buscarSeguradora(seguradoraString)));
		seguro.getListaCondutores().add(lerCondutor(Seguradora.buscarSeguradora(seguradoraString)));
	...
	public static void lerSeguroPJ(){ 
		...
		System.out.print("\nSeguradora:");
		String seguradoraString = lerStr();
		Seguro seguro = new SeguroPJ(id, LocalDate.parse(dataini), LocalDate.parse(datafim), Seguradora.buscarSeguradora(seguradoraString), lerFrota(lerVeiculo(Seguradora.buscarSeguradora(seguradoraString))), (ClientePJ) buscarClientePJ(cliente, Seguradora.buscarSeguradora(seguradoraString)));
		seguro.getListaCondutores().add(lerCondutor(Seguradora.buscarSeguradora(seguradoraString)));
		Seguradora.buscarSeguradora(seguradoraString).getListaSeguros().add(seguro);
	}
	...
	public static Veiculo lerVeiculo(Seguradora seguradora){
		...
		if(seguradora.buscarCliente(dono) instanceof ClientePF){
			((ClientePF) buscarClientePF(dono, Seguradora.buscarSeguradora(seguradoraString) )).getListaVeiculos().add( veiculo);
		}
			...
			subdescricao = SubmenuOpcoes.valueOf(lerStr());	;
				...
				{ 
					...
					case CADASTRAR_FROTA:
						System.out.print("\nPlaca:");
						String placa = lerStr();
						lerFrota(buscarVeiculo(placa, seguradora));
					break;
